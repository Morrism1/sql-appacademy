...
def lrt_stops
  ...
  execute(<<-SQL)
  SELECT id, name FROM stops JOIN routes ON stops.id = routes.stop_id
...
def connecting_routes
  ...
  FROM routes WHERE stop_id=149 OR stop_id=53
...
def cl_to_lr
  ...
  FROM routes a JOIN routes b ON
...
def haymarket_and_leith
  ...
  execute(<<-SQL)
  SELECT r1.company, r1.num FROM routes r1
  JOIN routes r2 ON (r1.num = r2.num AND r1.company = r2.company)
  WHERE 
  r1.stop_id IN (115)
  AND
  r2.stop_id IN (137)
...
def craiglockhart_to_sighthill
  ...
  start.stop_id IN(
  SELECT id FROM stops
  WHERE name = 'Craiglockhart') AND finish.stop_id IN (
  SELECT id FROM stops
