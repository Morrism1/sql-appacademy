...
def null_dept
  ...
  execute(<<-SQL)
  SELECT teachers.name FROM teachers LEFT JOIN depts ON depts.id = teachers.dept_id
  WHERE depts IS NULL
  SQL
end
...
def teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT name,COALESCE(NULL,mobile, '07986 444 2266') FROM teachers
  SQL
end
...
def teachers_and_depts
  ...
  # department.
  execute(<<-SQL)
  SELECT teachers.name, COALESCE(NULL,depts.name,'None')
  FROM teachers LEFT JOIN depts
           ON (teachers.dept_id=depts.id)
  SQL
end
...
def dept_staff_counts
  ...
  FROM teachers RIGHT JOIN depts
           ON (teachers.dept_id=depts.id)
  GROUP BY depts.name
  SQL
end
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
  SELECT name,
       (CASE WHEN dept_id IN (1,2) THEN 'Sci' WHEN dept_id = 3 THEN 'Art' ELSE 'None' END)
  FROM teachers;
  SQL
end
